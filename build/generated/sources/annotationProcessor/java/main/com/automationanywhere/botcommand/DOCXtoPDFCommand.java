...
public final class DOCXtoPDFCommand implements BotCommand {
  private static final Logger logger = LogManager.getLogger(DOCXtoPDFCommand.class);
  ...
  public Optional<Value> execute(GlobalSessionContext globalSessionContext,
    ...
    DOCXtoPDF command = new DOCXtoPDF();
    ...
    if(convertedParameters.containsKey("inputFile")) {
      ...
      if (lastIndxDot == -1 || lastIndxDot >= filePath.length()) {
        throw new BotCommandException(MESSAGES_GENERIC.getString("generic.validation.FileExtension","inputFile","docx"));
      }
      String fileExtension = filePath.substring(lastIndxDot + 1);
      if(!Arrays.stream("docx".split(",")).anyMatch(fileExtension::equalsIgnoreCase))  {
        throw new BotCommandException(MESSAGES_GENERIC.getString("generic.validation.FileExtension","inputFile","docx"));
      }
