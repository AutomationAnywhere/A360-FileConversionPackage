...
public final class ImagetoImageCommand implements BotCommand {
  private static final Logger logger = LogManager.getLogger(ImagetoImageCommand.class);
  ...
  public Optional<Value> execute(GlobalSessionContext globalSessionContext,
    ...
    ImagetoImage command = new ImagetoImage();
    ...
    if(convertedParameters.containsKey("inputFile")) {
      ...
      if (lastIndxDot == -1 || lastIndxDot >= filePath.length()) {
        throw new BotCommandException(MESSAGES_GENERIC.getString("generic.validation.FileExtension","inputFile","jpeg,jpg,gif,png,tiff"));
      }
      String fileExtension = filePath.substring(lastIndxDot + 1);
      if(!Arrays.stream("jpeg,jpg,gif,png,tiff".split(",")).anyMatch(fileExtension::equalsIgnoreCase))  {
        throw new BotCommandException(MESSAGES_GENERIC.getString("generic.validation.FileExtension","inputFile","jpeg,jpg,gif,png,tiff"));
      }
