...
public final class CSVtoXLSXCommand implements BotCommand {
  private static final Logger logger = LogManager.getLogger(CSVtoXLSXCommand.class);
  ...
  public Optional<Value> execute(GlobalSessionContext globalSessionContext,
    ...
    CSVtoXLSX command = new CSVtoXLSX();
    ...
    if(convertedParameters.containsKey("inputFile")) {
      ...
      if (lastIndxDot == -1 || lastIndxDot >= filePath.length()) {
        throw new BotCommandException(MESSAGES_GENERIC.getString("generic.validation.FileExtension","inputFile","csv"));
      }
      String fileExtension = filePath.substring(lastIndxDot + 1);
      if(!Arrays.stream("csv".split(",")).anyMatch(fileExtension::equalsIgnoreCase))  {
        throw new BotCommandException(MESSAGES_GENERIC.getString("generic.validation.FileExtension","inputFile","csv"));
      }
